...
// Recalcular status se o usuário trocar a quantização dinamicamente no card
const dynamicStatus = computed(() => {
  ...
  if (ramTotal >= ramNeeded + 1.5) {
    return {
      status: 'cpu_viable',
      status_label: isSmall ? 'Roda Fluido na CPU' : 'Roda na CPU',
      isCompatible: true,
...
<template>
  <div
    ...
    :class="{ 'opacity-65 hover:opacity-100': !dynamicStatus.isCompatible }"
  >
    <!-- Top Header of Card -->
    <div>
      <div class="flex items-start justify-between gap-3 mb-3">
        ...
        <div
          class="px-2.5 py-1 rounded-md text-xs font-semibold flex items-center gap-1.5 shrink-0 border"
          :class="dynamicStatus.status === 'gpu_perfect'
            ? 'bg-white text-black border-white'
            : dynamicStatus.isCompatible
            ? 'bg-zinc-800 text-zinc-200 border-zinc-700'
            : 'bg-zinc-950 text-zinc-500 border-zinc-800'"
        >
          <span
            class="w-1.5 h-1.5 rounded-full"
            :class="dynamicStatus.status === 'gpu_perfect' ? 'bg-black' : dynamicStatus.isCompatible ? 'bg-white' : 'bg-zinc-600'"
          ></span>
      ...
      <div class="grid grid-cols-2 gap-2 mb-4 p-2.5 rounded-xl bg-zinc-900/90 border border-zinc-800 text-xs">
        ...
        <div>
          ...
          <span class="font-semibold text-white truncate font-mono">
            {{ dynamicStatus.speed }}
          </span>
        </div>
      </div>
      ...
      <div
        ...
        class="mb-4 p-2.5 rounded-xl bg-zinc-900 border border-zinc-800 text-[11px] text-zinc-300"
      >
        <div class="flex items-start gap-1.5">
          <Lightbulb class="w-3.5 h-3.5 text-zinc-300 shrink-0 mt-0.5" />
